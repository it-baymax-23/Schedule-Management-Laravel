<form class="pl-3 pr-3" method="post" action="{{ route('notes.store',$currantWorkspace->slug) }}">
    ...
    <div class="form-group">
        ...
        <select class="form-control" name="color" required>
            <option value="bg-primary" class="bg-primary">{{ __('Primary') }}</option>
            <option value="bg-secondary" class="bg-secondary">{{ __('Secondary') }}</option>
            <option value="bg-info" class="bg-info">{{ __('Info') }}</option>
            <option value="bg-warning" class="bg-warning">{{ __('Warning') }}</option>
            <option value="bg-danger" class="bg-danger">{{ __('Danger') }}</option>
        </select>
    </div>
    <div class="form-group">
